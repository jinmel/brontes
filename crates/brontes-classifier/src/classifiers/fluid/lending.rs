...
action_impl!(
    ...
    [LogLiquidate],
    logs: true,
    |info: CallInfo, log_data: FluidLendingLiquidate_1CallLogs, db_tx: &DB, tracer: Arc<T>| {
        use std::pin::Pin;
        ...
        let logs = match log_data.log_liquidate_field {
            Ok(logs) => logs,
            Err(e) => return Box::pin(async move { Err(e) }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>,
        };
        
        let protocol_details = match db_tx.get_protocol_details(pool_address) {
            Ok(details) => details,
            Err(e) => return Box::pin(async move { Err(e) }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>,
        };
        ...
        let borrow_asset_token0 = match protocol_details.token2.ok_or_else(|| eyre::eyre!("Token3 does not exist")) {
            Ok(token) => token,
            Err(e) => return Box::pin(async move { Err(e) }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>,
        };
        
        let borrow_asset_token1 = match protocol_details.token3.ok_or_else(|| eyre::eyre!("Token4 does not exist")) {
            Ok(token) => token,
            Err(e) => return Box::pin(async move { Err(e) }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>,
        };
        ...
        let supply_info_token0 = match db_tx.try_fetch_token_info(supply_asset_token0) {
            Ok(info) => info,
            Err(e) => return Box::pin(async move { Err(e) }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>,
        };

        let borrow_info_token0 = match db_tx.try_fetch_token_info(borrow_asset_token0) {
            Ok(info) => info,
            Err(e) => return Box::pin(async move { Err(e) }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>,
        };
        ...
        // Return a boxed future that will complete the async operation
        Box::pin(async move {
            let exchange_rates = query_fluid_dex_state(&tracer, &pool_address, FLUID_DEX_RESOLVER_ADDRESS, is_smart_col, is_smart_borrow).await;
        ...
        }) as Pin<Box<dyn Future<Output = eyre::Result<NormalizedLiquidation>> + Send>>
    }
);
